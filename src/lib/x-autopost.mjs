...
export function evaluate(ev) {
  ...
  // SEC is no longer a permanent exile. A blanket block kept material 8-K disclosures — an
  // acquisition completing, a CFO leaving, a bankruptcy petition, unregistered equity going out the
  // door — off an account that exists to carry exactly those, and did it on the basis of the SOURCE
  // rather than the event. The source still never qualifies anything: an SEC event goes through the
  // same catalyst rules as everything else and is posted only if it independently passes them.
  //
  // Nothing about SEC ingestion, storage or parsing changes. This reads the rows the filing feed
  // already wrote and decides, downstream, whether one of them is worth a post.
  ...
  // An EDGAR index line is mechanical in exactly the way a halt notice is. "CHIPOTLE MEXICAN GRILL
  // INC · 8-K (5.02,9.01)" is assembled from the filing's own fields — registrant, form type, item
  // numbers — and has no author whose prose could be misrepresented. The wording gate exists to stop
  // a PUBLISHER's sentence going out as Catalyst Pit's; there is no sentence here to take. The post
  // text is composed from the SEC's own item taxonomy, not from the filing's language.
  const mechanical = (String(ev?.source_type || '') === 'halt' || String(ev?.source_type || '') === 'filing')
  ...
  if (mechanical) {
    return { eligible: true, blocked: null,
      reason: String(ev?.source_type || '') === 'filing' ? 'sec filing' : 'exchange halt' };
  }
...
// EDGAR files registrant names in block capitals ("CHIPOTLE MEXICAN GRILL INC"). Shouting them on
// a public account reads as a machine emptying a queue, so an all-caps name is title-cased. A name
// already in mixed case is left exactly as filed — "byNordic Acquisition Corp" is spelled that way
// on purpose — and short all-caps tokens that are initialisms are preserved.
const KEEP_CAPS = /^(?:[A-Z]{1,4}|[A-Z]+\d[A-Z\d]*)$/;
export function titleCaseRegistrant(name) {
  const s = String(name || '').trim();
  if (!s || s !== s.toUpperCase()) return s;              // mixed case: as filed
  return s.split(/\s+/).map((w) => (KEEP_CAPS.test(w) ? w
...
export function formatPost(ev, reading = null, now = Date.now(), breaking = null, catalyst = null) {
  ...
  // An 8-K index line is not a sentence. "CHIPOTLE MEXICAN GRILL INC · 8-K (5.02,9.01)" published
  // verbatim would be a form number and a list of codes; what a reader needs is what the item MEANS,
