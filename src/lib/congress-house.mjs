...
export async function extractPdfText(buf) {
  ...
  // unpdf emits NUL / C0 control chars for letter-spaced glyphs; Postgres text can't store .
...
export function parsePtrTransactions(rawText) {
  ...
  while ((m = CORE.exec(region))) {
    ...
    // Multi-sale filings prepend a long "Filing Status/Subholding/Description" block; keep the
    // tail (the actual asset name sits just before the ticker) and drop a partial leading word.
    if (asset.length > 80) asset = asset.slice(-80).replace(/^\S*\s+/, '');
