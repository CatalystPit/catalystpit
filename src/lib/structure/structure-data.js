...
export async function loadDailyHistory(ticker, { asOf = null, days = HISTORY_DAYS } = {}) {
...
export async function loadPriceQuality(ticker) {
  const symbol = String(ticker || '').toUpperCase();
  try {
    const res = await db.execute(sql`
...
export async function tickerStructure(ticker, { asOf = null } = {}) {
  const [bars, quality] = await Promise.all([
    loadDailyHistory(ticker, { asOf }),
    loadPriceQuality(ticker),
  ]);
