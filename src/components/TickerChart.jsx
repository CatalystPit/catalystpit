...
function insiderSection(trades, labeled) {
  const head = labeled ? `<div style="font-size:8px;letter-spacing:0.8px;color:${C.dim};font-family:'DM Sans',sans-serif;margin-bottom:2px">INSIDER</div>` : '';
  if (trades.length === 1) {
...
function congressSection(trades, labeled) {
  const head = labeled ? `<div style="font-size:8px;letter-spacing:0.8px;color:${C.dim};font-family:'DM Sans',sans-serif;margin-bottom:2px">CONGRESS</div>` : '';
  if (trades.length === 1) {
    const t = trades[0], col = t.action === 'BUY' ? GREEN : RED;
  ...
  const btn = (label, active, onClick) => (
    <button key={label} className="hov" onClick={onClick}
      style={{ background: 'transparent', border: 'none', cursor: 'pointer', padding: '4px 10px', borderRadius: 4,
        fontFamily: "'DM Sans',sans-serif", fontSize: 12, color: active ? C.ink : C.muted, fontWeight: active ? 700 : 400,
  ...
  return (
    <div style={{ marginTop: 14, background: C.white, border: `1px solid ${C.border}`, borderRadius: 10, overflow: 'hidden' }}>
      ...
      <div style={{ padding: 14 }}>
        ...
        <div style={{ marginTop: 8, fontFamily: "'DM Sans',sans-serif", fontSize: 10, color: C.dim, letterSpacing: '0.3px' }}>
