...
// Affiliate offers (C6) — env-driven so no partner is hardcoded and the strip renders
// NOTHING until IDs are configured. Set full destination URLs (client-visible, so
// NEXT_PUBLIC_*): NEXT_PUBLIC_AFF_TRADINGVIEW / _BROKER / _UNUSUAL_WHALES. FTC disclosure
// is always shown when any link renders. rel="nofollow sponsored" per affiliate best practice.
const OFFERS = [
...
export default function AffiliateStrip({ compact = false }) {
  ...
  if (!offers.length) return null;      // nothing configured → render nothing
