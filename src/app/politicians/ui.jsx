...
export function Chip({ children, bg, fg }) {
  return (
    <span style={{ fontFamily: "var(--font-dm-mono),monospace", fontSize: 10, fontWeight: 600, letterSpacing: '0.4px',
...
export function Avatar({ photoUrl, name, ps, size = 52 }) {
  ...
  return (
    <div style={{ width: size, height: size, borderRadius: '50%', flexShrink: 0, display: 'flex',
      ...
      fontFamily: "var(--font-dm-mono),monospace", fontWeight: 700, fontSize: Math.round(size * 0.3) }}>
      {initials(name)}
    </div>
  );
}
...
export function Stat({ label, value, small }) {
  return (
    <div>
      <div className="cp-num" style={{ fontFamily: "var(--font-dm-mono),monospace", fontWeight: 600,
        fontSize: small ? 12 : 16, color: C.ink }}>{value}</div>
      <div style={{ fontFamily: "var(--font-dm-mono),monospace", fontSize: 9, color: C.dim, letterSpacing: '0.8px', marginTop: 2 }}>{label}</div>
    </div>
  );
}
