  ...
  return (
    <div style={{ fontFamily: "'DM Sans',sans-serif", background: C.bg, color: C.text, minHeight: '100vh' }}>
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '20px 24px 40px' }}>
        <a href="/politicians" style={{ fontFamily: "var(--font-dm-mono),monospace", fontSize: 12, letterSpacing: '0.8px', color: C.green, textDecoration: 'none' }}>← All politicians</a>
        ...
        {error ? (
          ...
          <>
            ...
            {/* TRADE HISTORY TABLE */}
            <div style={{ fontSize: 12, fontWeight: 400, color: C.dim, fontFamily: "var(--font-dm-mono),monospace", letterSpacing: '0.8px', margin: '18px 0 10px' }}>{trades.length} trades</div>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 760 }}>
                  <thead>
                    <tr style={{ background: C.surface, borderBottom: `1px solid ${C.border}` }}>
                      {COLS.map(([h, al]) => (
                        <th key={h} style={{ padding: '10px 14px', textAlign: al, fontFamily: "var(--font-dm-mono),monospace", fontSize: 9, color: C.dim, letterSpacing: '0.8px', fontWeight: 400, whiteSpace: 'nowrap' }}>{h.toUpperCase()}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {trades.length === 0 ? (
                      ...
                      return (
                        <tr key={t.id || i}
                          className={t.ticker ? 'hov' : undefined}
                          onClick={t.ticker ? () => router.push(`/ticker/${encodeURIComponent(t.ticker)}`) : undefined}
                          style={{ borderBottom: i < trades.length - 1 ? `1px solid ${C.surface}` : 'none', borderLeft: `3px solid ${as.fg}` }}>
                          <td className="cp-tkr" style={{ padding: '12px 14px', fontFamily: "var(--font-dm-mono),monospace", fontSize: 13, fontWeight: 700, color: t.ticker ? C.green : C.dim, whiteSpace: 'nowrap' }}>{t.ticker || '—'}</td>
                          ...
                          <td style={{ padding: '12px 14px', fontFamily: "var(--font-dm-mono),monospace", fontWeight: 400, letterSpacing: '0.8px', fontSize: 11, color: C.muted, whiteSpace: 'nowrap' }}>{fmtDate(t.transactionDate)}</td>
                          <td style={{ padding: '12px 14px', fontFamily: "var(--font-dm-mono),monospace", fontWeight: 400, letterSpacing: '0.8px', fontSize: 11, color: C.muted, whiteSpace: 'nowrap' }}>{fmtDate(t.disclosureDate)}</td>
                          <td style={{ padding: '12px 14px', textAlign: 'right', fontFamily: "var(--font-dm-mono),monospace", fontSize: 12, color: late ? C.red : C.muted, whiteSpace: 'nowrap' }}>{t.filingLagDays != null ? `${t.filingLagDays}d` : '—'}</td>
                          <td style={{ padding: '12px 14px' }}>
                            <span style={{ fontSize: 11, padding: '3px 9px', borderRadius: 4, fontFamily: "var(--font-dm-mono),monospace", fontWeight: 600, background: as.bg, color: as.fg }}>{t.action}</span>
                          </td>
                          <td className="cp-num" style={{ padding: '12px 14px', textAlign: 'right', fontFamily: "var(--font-dm-mono),monospace", fontSize: 12, color: C.text, whiteSpace: 'nowrap' }}>{t.amountRange || '—'}</td>
                          <td className="cp-num" style={{ padding: '12px 14px', textAlign: 'right', fontFamily: "var(--font-dm-mono),monospace", fontSize: 13, fontWeight: 700, color: returnColor(t.returnPct), whiteSpace: 'nowrap' }}>{fmtReturn(t.returnPct)}</td>
                        </tr>
                      );
                    })}
                  </tbody>
            ...
            <div style={{ marginTop: 16, fontSize: 11, color: C.dim, fontFamily: "var(--font-dm-mono),monospace", lineHeight: 1.6 }}>
