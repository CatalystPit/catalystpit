...
function SentimentChart({ data }) {
  const days = data || [];
  if (!days.length) return <div style={{fontSize:12,color:C.dim,fontFamily:"var(--font-dm-mono),monospace"}}>No sentiment data.</div>;
    ...
    <div style={{fontFamily:"'DM Sans',sans-serif",background:C.bg,color:C.text,minHeight:"100vh"}}>
        ...
        <div style={{maxWidth:1380,margin:"0 auto",display:"flex",justifyContent:"space-between",alignItems:"flex-end",flexWrap:"wrap",gap:12}}>
          <div>
            <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:6}}>
              <Dot/><span style={{fontFamily:"var(--font-dm-mono),monospace",fontSize:10,color:C.muted,letterSpacing:"1px"}}>FORM 4 · SEC EDGAR · LIVE</span>
            </div>
            ...
            {isTradeView && (
              <>
                <div style={{background:C.greenLight,border:`1px solid ${C.greenBorder}`,borderRadius:8,padding:"10px 18px",textAlign:"center"}}>
                  <div className="cp-num" style={{fontFamily:"var(--font-dm-mono),monospace",fontSize:20,fontWeight:600,color:C.green}}>{loading?'—':buys}</div>
                ...
                <div style={{background:C.redLight,border:`1px solid #E0AAAA`,borderRadius:8,padding:"10px 18px",textAlign:"center"}}>
                  <div className="cp-num" style={{fontFamily:"var(--font-dm-mono),monospace",fontSize:20,fontWeight:600,color:C.red}}>{loading?'—':sells}</div>
                  <div style={{fontSize:11,color:C.red,fontWeight:500}}>SELLS</div>
                </div>
              </>
            )}
            <div className="cp-num" style={{fontFamily:"var(--font-dm-mono),monospace",fontSize:10,color:C.dim}}>
              Updated {timeStr}
              <button onClick={refresh} style={{background:"transparent",border:"none",color:C.green,cursor:"pointer",fontSize:12,marginLeft:8,fontFamily:"var(--font-dm-mono),monospace"}}>↻</button>
            </div>
          </div>
        </div>
      </div>
      ...
      <div style={{maxWidth:1380,margin:"0 auto",padding:"16px 24px 0",display:"flex",flexDirection:"column",gap:14}}>
        <div style={{display:"flex",alignItems:"center",gap:10,flexWrap:"wrap"}}>
          ...
          {searching && (
            <span style={{display:"inline-flex",alignItems:"center",gap:8,background:C.greenLight,border:`1px solid ${C.greenBorder}`,borderRadius:5,padding:"6px 12px",fontFamily:"var(--font-dm-mono),monospace",fontSize:12,color:C.green}}>
              {debouncedSearch}
          ...
          {CATEGORIES.map(cat => {
            ...
            return (
              <button key={cat.key} className="cat" onClick={()=>selectView(cat.key)}
                ...
                <div style={{fontFamily:"var(--font-dm-mono),monospace",fontSize:12,fontWeight:600,letterSpacing:"0.5px",color:active?"#fff":C.ink}}>{cat.label}</div>
                <div style={{fontFamily:"var(--font-dm-mono),monospace",fontSize:10,letterSpacing:"0.8px",marginTop:3,color:active?"rgba(255,255,255,0.8)":C.dim}}>{cat.sub}</div>
              </button>
            );
          })}
        </div>
      ...
      <div style={{maxWidth:1380,margin:"16px auto",padding:"0 24px 40px"}}>
        {error ? (
          ...
          <div style={{display:"flex",flexDirection:"column",gap:16}}>
            <div style={{background:C.white,border:`1px solid ${C.border}`,borderRadius:8,padding:"16px 18px"}}>
              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:12}}>
                ...
                <span style={{fontFamily:"var(--font-dm-mono),monospace",fontSize:10,letterSpacing:"0.8px",color:C.dim}}>
              ...
              {(data.trending||[]).map((t,i)=>(
                <div key={i} style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"11px 16px",borderBottom:i<data.trending.length-1?`1px solid ${C.surface}`:"none"}}>
                  <div style={{display:"flex",gap:12,alignItems:"baseline"}}>
                    <span style={{fontFamily:"var(--font-dm-mono),monospace",fontSize:13,fontWeight:700,color:C.green,minWidth:64}}>{t.ticker}</span>
                  ...
                  <div style={{fontFamily:"var(--font-dm-mono),monospace",fontSize:12}}>
            ...
            <table style={{width:"100%",borderCollapse:"collapse"}}>
              <thead><tr style={{background:C.surface,borderBottom:`1px solid ${C.border}`}}>
                {["Ticker","Company","Buyers","Trades","Total $","Window"].map((h,i)=>(
                  <th key={h} style={{padding:"10px 16px",textAlign:i>=2&&i<=4?"right":"left",fontFamily:"var(--font-dm-mono),monospace",fontSize:9,color:C.dim,letterSpacing:"0.8px",fontWeight:400}}>{h.toUpperCase()}</th>
                ))}
              </tr></thead>
              <tbody>
                {(data.clusters||[]).length===0 ? (
                  ...
                  <tr key={i} className="row-hov" onClick={()=>goTicker(c.ticker)} style={{borderBottom:i<data.clusters.length-1?`1px solid ${C.surface}`:"none",borderLeft:`3px solid ${C.green}`}}>
                    <td className="cp-tkr" style={{padding:"13px 16px",fontFamily:"var(--font-dm-mono),monospace",fontSize:13,fontWeight:700,color:C.green}}>{c.ticker}</td>
                    ...
                    <td className="cp-num" style={{padding:"13px 16px",textAlign:"right",fontFamily:"var(--font-dm-mono),monospace",fontSize:14,fontWeight:700,color:C.green}}>{c.buyers}</td>
                    <td className="cp-num" style={{padding:"13px 16px",textAlign:"right",fontFamily:"var(--font-dm-mono),monospace",fontSize:13,color:C.text}}>{c.trades}</td>
                    <td className="cp-num" style={{padding:"13px 16px",textAlign:"right",fontFamily:"var(--font-dm-mono),monospace",fontSize:13,fontWeight:600,color:C.text}}>{fmtMoney(c.totalValue)}</td>
                    <td className="cp-num" style={{padding:"13px 16px",fontFamily:"var(--font-dm-mono),monospace",fontSize:11,color:C.dim,whiteSpace:"nowrap"}}>{c.firstBuy} → {c.lastBuy}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <>
            <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:10,fontSize:12,color:C.dim,fontFamily:"var(--font-dm-mono),monospace",letterSpacing:"0.8px"}}>
              ...
              <table style={{width:"100%",borderCollapse:"collapse"}}>
                <thead><tr style={{background:C.surface,borderBottom:`1px solid ${C.border}`}}>
                  {[
                    ...
                    return <th key={h.label} onClick={h.sortKey?()=>handleSort(h.sortKey):undefined} style={{padding:"10px 16px",textAlign:h.align||"left",fontFamily:"var(--font-dm-mono),monospace",fontSize:9,color:active?C.green:C.dim,letterSpacing:"0.8px",fontWeight:400,cursor:h.sortKey?"pointer":"default",userSelect:"none"}}>{h.label.toUpperCase()}{arrow}</th>;
                  })}
                </tr></thead>
                <tbody>
                  {rows.length===0 ? (
                    ...
                    <tr key={i} className="row-hov" onClick={()=>goTicker(ins.sym)} style={{borderBottom:i<rows.length-1?`1px solid ${C.surface}`:"none",borderLeft:`3px solid ${actionStyles(ins.type).fg}`}}>
                      <td className="cp-num" style={{padding:"13px 16px",fontFamily:"var(--font-dm-mono),monospace",fontSize:11,color:C.dim,whiteSpace:"nowrap"}}>{ins.filed}</td>
                      <td className="cp-tkr" style={{padding:"13px 16px",fontFamily:"var(--font-dm-mono),monospace",fontSize:13,fontWeight:700,color:C.green}}>{ins.sym}</td>
                      ...
                      <td style={{padding:"13px 16px"}}>
                        <span style={{fontSize:11,padding:"4px 10px",borderRadius:4,fontFamily:"var(--font-dm-mono),monospace",fontWeight:600,letterSpacing:"0.5px",background:actionStyles(ins.type).bg,color:actionStyles(ins.type).fg}}>{ins.type}</span>
                      </td>
                      <td className="cp-num" style={{padding:"13px 16px",textAlign:"right",fontFamily:"var(--font-dm-mono),monospace",fontSize:13,fontWeight:500,color:C.text,whiteSpace:"nowrap"}}>{ins.shares>0?ins.shares.toLocaleString('en-US'):'—'}</td>
                      <td className="cp-num" style={{padding:"13px 16px",textAlign:"right",fontFamily:"var(--font-dm-mono),monospace",fontSize:13,fontWeight:500,color:C.muted,whiteSpace:"nowrap"}}>{fmtPrice(ins.avgPrice)}</td>
                      <td className="cp-num" style={{padding:"13px 16px",textAlign:"right",fontFamily:"var(--font-dm-mono),monospace",fontSize:14,fontWeight:700,color:actionStyles(ins.type).fg}}>{ins.value}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
