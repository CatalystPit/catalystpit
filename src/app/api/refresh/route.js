  ...
  const results = await Promise.all(
    tickers.map(async sym => {
      try {
        ...
        const data = await res.json();
        const t = data.ticker || data.results?.ticker || data.results;
        if (!t) {
          console.log(`⚠️ Polygon ${sym}: no ticker. Keys: ${Object.keys(data).join(',')}`);
          return null;
        }
        const price = t.lastTrade?.p ?? t.min?.c ?? t.day?.c ?? t.prevDay?.c;
        if (!price) {
          console.log(`⚠️ Polygon ${sym}: no price. Ticker keys: ${Object.keys(t).join(',')}`);
          return null;
        }
