...
// cached in the warm instance. Prefix matches on the symbol rank first (so "NV" → NVDA), then
// symbol-contains, then company-name-contains. Returns up to 8 { ticker, name }.
...
async function loadTickers() {
  ...
  try {
    ...
    const arr = Object.values(data).map((x) => ({ ticker: String(x.ticker).toUpperCase(), name: x.title || '' }));
    CACHE = arr;
    CACHE_AT = Date.now();
    return arr;
  } catch {
