...
export async function GET(request) {
  ...
  try {
    ...
    try { s13d = await ingestSchedule13D(); } catch { /* best effort */ }
    ...
    await recordJobRun('eightk', { ok: true, seen: (res.inserted ?? 0) + (f25.inserted ?? 0) + (f144.inserted ?? 0) + (s13d.inserted ?? 0), note: `scanned ${res.scanned} 8-K, ${f25.scanned} form-25, ${f144.scanned} form-144, ${s13d.scanned} sched-13d` });
    return Response.json({ ok: true, ...res, form25: f25, form144: f144, schedule13d: s13d });
  } catch (e) {
