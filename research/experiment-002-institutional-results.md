...
| `buy_only + accumulation` n | 137 | **1,279** |
...
|---|---:|---:|---:|
| n | 137 | 1,279 | 1,064 |
| cohort-relative median | +3.53% | +2.66% | +1.67% |
| hit rate | 58% | 56% | 53% |
| increment over insider-alone | +1.18% | **+0.29%** | — |

Insider `buy_only` with a 13F available is +2.37% on n=2,358. Adding institutional accumulation moves
that to +2.66%. **The increment is +0.29%**, on t=0.82 with an effective N of about 20 after the 63x
overlap deflation. Adding "mixed" gives +0.28% — indistinguishable from accumulation, which is what
...
`buy_only + distribution` is the only configuration whose increment is large: **-3.29%** against the
same baseline. Against it: n=187, t=0.10, effective N of about 3. On the protected holdout the same
cell is +0.28% on n=333 — it does not reproduce.

The honest reading is **UNDERPOWERED, not supported**. 754 distribution observations exist against
7,193 accumulation, because institutions in aggregate accumulate far more often than they distribute
...
Out-of-sample, fold 0: accumulation **-3.72%** (n=3,040, hit 44%), distribution **-7.23%** (n=267,
hit 37%). The protected holdout, read once: accumulation -0.63% (n=5,129), distribution +1.62%
(n=913) — the sign on distribution flips between exploratory and holdout, which is what noise does.
